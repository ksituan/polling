...
function DashProvince({jurisdiction}) {

    let plotWidth = 400;
    let plotHeight = 350;
    let padding = 50;

    // Find newest election

    let elecs = elections.content[jurisdiction];

    elecs = elecs.sort((a,b) => new Date(b.date) - new Date(a.date));

    let election = elecs[0];
    ...
    return(
        ...
        <div className="dashProvTitle">{title[jurisdiction]}</div>
        </div>
    );
}
